...
def inception_block(A_prev, filters):
    ...
    F1, F3R, F5R, F5, FPP = filters
    conv1 = K.layers.Con2D(filters=F1, kernel_size=(1, 1),
                           padding='same', activation='relu')(A_prev)
    conv3R = K.layers.Con2D(filters=F3R, kernel_size=(1, 1),
                            padding='same', activation='relu')(A_prev)
    ...
    output = K.layers.Conv2D.concatenate((conv1, conv3, conv5, convPP))
    return output
